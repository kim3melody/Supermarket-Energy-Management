...
# Main Script
path=r'C:\Users\czhang0914\Desktop\Python Test Week 25'
os.chdir(path)
...
Date=data.index
Week=Date.map(lambda x: str(x.year)+'-'+str((x+datetime.timedelta(days=1)).isocalendar()[1]+1))
Day=Date.map(lambda x: x.dayofweek)
Day_Night=Date.map(lambda x: 'night' if x.hour<7 else 'day')

# Loop through the site list, and pump groups
for i in site:
    temp=pd.DataFrame(index=Date)
    temp['Week']=Week; temp['day/night']=Day_Night; temp['Day']=Day 
    n=0 # Pump Group index
    summary[i]={}
    for pg in site[i]: 
        # tariff rules
        t=tariff[i]
        temp['New tariff']=Date.map(lambda x:  t[0] if x.dayofweek>=5 and x.hour<7                       # Sat, Sun before 7   => t[0]
...
#       temp['KWH/ML '+sn]=(temp['KWH '+sn]/ temp['ML '+sn]).where(temp['ML '+sn]>0)
        ...
        summary[i][n]=temp.groupby(['Week']).agg({'ML '+sn:lambda x: sum(x),'Cost '+sn:lambda x:sum(x)})
...
#summary['wg'][2]=summary['wg'][2].rename(columns={'ML 2':'ML 4','Cost 2':'Cost 4'})
