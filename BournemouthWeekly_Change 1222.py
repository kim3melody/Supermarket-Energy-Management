...
# Main Script
path=r'C:\Users\czhang0914\Desktop\KG\20170131\week34\week34'
os.chdir(path)
...
for i in site:
    ...
    for pg in site[i]: 
        ...
        for p in pg[1]: 
            ...
            temp[Operation] += temp[p].map(lambda x: 1 if x>8 else 0) 
            temp[type] += temp[p].map(lambda x: str(pi)+'/' if x>1 else '-/')
...
res['wg']=wg
daily_summary['wg'][3]=wg.groupby(['Date']).agg({'Total cost': lambda x: sum(x)})
...
weekly_summary['wg'][3]=wg.groupby(['Week']).agg({'Total cost': lambda x: sum(x)})
...
# ===========Longham format ===============
res['Longham'] = res['Longham'].drop(['Operation 1','type 1'],axis=1)
