...
#i_report = stats.loc[(stats['StateHistory First Record Date']>='20130601') &(stats['StateHistory First Record Date']<= '20160531')]
#i_report = i_report.groupby(['ECM Category', 'Type Code'],as_index = False).agg({'Site ID': lambda x: len(x.value_counts()),
#                                                                                'Code': lambda x: len(x),
#                                                                                'Estimated Consumption kWh':lambda x:sum(x),
#                                                                                 'Estimated Cost £':lambda x:sum(x)                                                                                    
#                                                                                })
#i_report.to_excel('i_report.xlsx')
